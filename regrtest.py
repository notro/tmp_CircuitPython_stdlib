...
class os:
    ...
    def listdir(path):
        cmd = "__import__('_os').listdir({!r})".format(path)
        res = board.eval(cmd, reset_repl=False)
        return res
...
def _create_parser():
    ...
    group.add_argument('--board', help='build_name, vid:pid or /dev/tty')       ###
...
#    ns = _parse_args(sys.argv[1:], **kwargs)
    ns = _parse_args(_sys.argv[1:], **kwargs)                                   ###
...
#                msvcrt.CrtSetReportFile(m, msvcrt.CRTDBG_FILE_STDERR)
    ...
    global board                                                                ###
    board = cpboard.CPboard(ns.board)                                           ###
    if ns.verbose:                                                              ###
        print()                                                                 ###
        print('board', board)                                                   ###
                                                                                ###
...
@cpboard.remote                                                                 ###
def board_findtests(testdir=None, stdtests=None, nottests=None):                ###
    from test import regrtest                                                   ###
    return regrtest.findtests(testdir=testdir, stdtests=stdtests, nottests=nottests)  ###
                                                                                ###
def findtests(testdir=None, stdtests=STDTESTS, nottests=NOTTESTS):
    return board_findtests(board, testdir, stdtests, nottests, _reset_repl=False)  ###
...
@cpboard.remote                                                                 ###
def board_runtest(test, verbose, quiet,                                         ###
            huntrleaks=False, use_resources=None,                               ###
            output_on_failure=False, failfast=False, match_tests=None,          ###
            timeout=None):                                                      ###
    from test import regrtest                                                   ###
    return regrtest.runtest(test, verbose, quiet,                               ###
