...
""".replace('{0[0]}', '{0}').replace('{0[1]}', '{1}').replace('{1[0]}', '{2}').replace('{1[1]}', '{3}').format(self.delimiters[0], self.delimiters[1], self.comment_prefixes[0], self.comment_prefixes[1])  ### NotImplementedError: attributes not supported yet
#""".format(self.delimiters, self.comment_prefixes)
        ...
        if self.strict:
            with self.assertRaises(configparser.DuplicateOptionError):
                ...
                """.replace('{0[0]}', '{0}').replace('{0[1]}', '{1}').format(self.delimiters[0], self.delimiters[1])))  ###
...
#    @unittest.skip('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX')
...
#            "test {0[1]} we        {comment} can\n"
            "test {1} we        {comment} can\n"                                ###
            "            also      {comment} place\n"
            "            comments  {comment} in\n"
            "            multiline {comment} values"
#            "\n".format(self.delimiters, comment=self.comment_prefixes[0],
            "\n".format(self.delimiters[0], self.delimiters[1], comment=self.comment_prefixes[0],  ###
                        default_section=self.default_section)
            )
        if self.allow_no_value:
            config_string += (
    ...
    def check_items_config(self, expected):
        ...
        """.replace('{0[0]}', '{0}').replace('{0[1]}', '{1}').format(self.delimiters[0], self.delimiters[0]), defaults={"default": "<default>"})  ###
#        """.format(self.delimiters), defaults={"default": "<default>"})
    ...
    def test_popitem(self):
        cf = self.fromstring("""
            ...
            [section3]
            name3 {0[0]} value3
        """.replace('{0[0]}', '{0}').format(self.delimiters[0]), defaults={"default": "<default>"})  ###
#        """.format(self.delimiters), defaults={"default": "<default>"})
    ...
    def test_setitem(self):
        cf = self.fromstring("""
            ...
            [section3]
            name3 {0[0]} value3
        """.replace('{0[0]}', '{0}').format(self.delimiters[0]), defaults={"nameD": "valueD"})  ###
...
@unittest.skip('Missing collections.UserDict')                                  ###
